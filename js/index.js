...
getWeather('cairo');
//? ==> function to display weather of current day
  ...
  cardGroup.innerHTML = `
           ...
           <div class="card-body">
             ...
             <div class="d-flex mt-3 text-secondary">
               <p class="d-flex justify-content-between align-items-center me-3"> <img src="images/icon-umberella.png" alt="" class="me-2"><span>20%</span>
               </p>
               <p class="d-flex justify-content-between align-items-center me-3"> <img src="images/icon-wind.png" alt=""class="me-2"><span>18km/h</span>
               </p>
               <p class="d-flex justify-content-between align-items-center me-3"> <img src="images/icon-compass.png" alt="" class="me-2"><span>East</span>
               </p>
             </div>
           </div>
         </div>`;
...
//* ==> function to display weather of future day

function displayFuture(fut) {
  for (let i = 1; i < 3; i++) {
    cardGroup.innerHTML += `
    <div class="card border-0">
        <div class="card-header bg-black bg-opacity-25 border-0 text-center text-light">
            <span id="dayTwo">${days[new Date(fut.forecastday[i].date).getDay()]}</span>
        </div>
        <div class="card-body d-flex flex-column justify-content-around align-items-center py-5">
            <img src="https:${fut.forecastday[i].day.condition.icon}" alt="icon-weather">
                <div>
                    <h4 class="text-white">${fut.forecastday[i].day.maxtemp_c}<sup>o</sup>C</h4>
                    <h6 class="text-secondary text-center">${
                    fut.forecastday[i].day.mintemp_c
                    }<sup>o</sup></h6>
                </div>
            <span>${fut.forecastday[i].day.condition.text}</span> 
        </div>
    </div>
...
//! set inputSearch value in function  api to display country
